...
- The dev shell provides a flake-owned `bd` wrapper, `tusk-flake-ref`, `tusk-tracker`, `tusk-codex`, `tusk-skill-contract-check`, `tusk-skill-loop`, `dolt`, `jj`, `deadnix`, `statix`, `nil`, `nixd`, `nix-tree`, `nix-output-monitor`, `nixfmt`, `codex`, `tuskd`, `glistix`, `gleam`, `erl`, `rebar3`, a `rust-overlay` toolchain for `cargo`/`rustc`/`rustfmt`, and `rust-analyzer`.
...
- `flake.nix` also exports `tusk-codex`, `tusk-skill-contract-check`, and `tusk-skill-loop` as the repo-owned launcher, validation surface, and fast-restart authoring loop for shared skills.

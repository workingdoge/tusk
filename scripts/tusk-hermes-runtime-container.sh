...
require_env() {
  ...
  if [ -z "${!name:-}" ]; then
    printf 'tusk-hermes-runtime-container: missing required env: %s\n' "${name}" >&2
    exit 1
  fi
}
...
require_env TUSK_HERMES_RUNTIME_COMMAND
require_env TUSK_HERMES_RUNTIME_INTERACTIVE
...
printf '%s\n' "${TUSK_HERMES_RUNTIME_COMMAND}" >"/artifacts/command.txt"

if [ "${TUSK_HERMES_RUNTIME_INTERACTIVE}" = "1" ]; then
  exec bash -c "${TUSK_HERMES_RUNTIME_COMMAND}"
fi

(
  set -x
  bash -c "${TUSK_HERMES_RUNTIME_COMMAND}"
) >"/artifacts/command.log" 2>&1
